...
def load_obj_mesh_mtl(mesh_file):
	...
	for line in f:
		...
		if values[0] == 'v':
			...
			if len(values) > 4:
				f = list(
					map(lambda x: int(x.split('//')[0]) if int(x.split('//')[0]) < 0 else int(x.split('//')[0]) - 1,
						values[1:4]))
...
vertices, faces, normals, face_normals, textures, face_textures = load_obj_mesh('/Users/ckxz/Desktop/untitled.obj',
																				with_normal=True, with_texture=True)

print(
	f'vertices:  {vertices.shape}, faces: {faces.shape}, normals: {normals.shape}, face_normals: {face_normals.shape}, textures: {textures.shape}, face_textures: {face_textures.shape}')
