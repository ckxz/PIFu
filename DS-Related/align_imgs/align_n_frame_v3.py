...
def process_image(img, landmarks, dst, folder, rep, filename, output_size=1024):
	...
	#border_x = int(np.rint((_x2_ - _x1_) * 0.4))
	border_x = 0
	...
	if rep == 'Statue':
		img_ = Image.fromarray(np.uint8(cropped_image)).convert('RGB')
		img_ = img_.resize((cropped_image.shape[0], output_size), Image.ANTIALIAS)
		#img_.show()
		img_.save(f'{dst}/{folder}/{rep}/{filename}.png')
	else:
		img_ = Image.fromarray(np.uint8(cropped_image)).convert('RGB')
		img_ = img_.resize((output_size, output_size), Image.ANTIALIAS)
		#img_.show()
		img_.save(f'{dst}/{folder}/{rep}/{filename}.png')
